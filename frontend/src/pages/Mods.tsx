...
import { deleteFile, fetchFiles, renameFile, uploadFile, type FileEntry } from "../api/files";
...
export function Mods({ server }: Props) {
  ...
  async function handleUpload(files: FileList | null) {
    ...
    try {
      for (const file of fileList) {
        await uploadFile(server.id, MODS_PATH, file);
      }
      await load();
    } catch {
  ...
  async function handleDelete(row: ModRow) {
    ...
    setError(null);
    try {
      await deleteFile(server.id, `${MODS_PATH}/${row.entry.name}`);
      await load();
    } catch {
      setError("削除に失敗しました。");
    } finally {
